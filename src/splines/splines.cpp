...
float idSplineList::calcSpline(int step, float tension) {
    ...
    case 2:
        return (-3 * pow(tension, 3) + 3 * pow(tension, 2) + 3 * tension + 1) /
               6;
    case 3:
        ...
        if (time >= startTime + events[i]->getTime() &&
            ...
            if (events[i]->getType() == idCameraEvent::EVENT_TARGET) {
                ...
                if (param2) {
                    ...
                    loadCamera(0,
                               va("cameras/%s.camera", events[i]->getParam()));
                    startCamera(time);
                }
                return true;
...
void idCameraDef::buildCamera() {
    ...
    for (i = 0; i < events.Num(); i++) {
        ...
        case idCameraEvent::EVENT_FEATHER: {
            ...
            long loopTime = 10;
            float stepGoal =
                cameraPosition->getBaseVelocity() / (1000 / loopTime);
        ...
        case idCameraEvent::EVENT_WAIT: {
            ...
            long loopTime = 10;
            float steps =
                speed / ((events[i]->getTime() - startTime) / loopTime);
        ...
        case idCameraEvent::EVENT_SPEED: {
            /*
                            // take the average delay between up to the next
               five segments float adjust = atof(events[i]->getParam()); int
               index = events[i]->getSegment(); total = 0; count = 0;

                            // get total amount of time over the remainder of
               the segment for (j = index; j < cameraSpline.numSegments() - 1;
               j++) { total += cameraSpline.getSegmentTime(j + 1) -
               ...
               { cameraSpline.addSegmentTime(j, additive); additive += newTotal;
                            }
                            break;
            */
        default:
...
void idCameraDef::save(const char* filename) {
    ...
    if (file) {
        ...
        for (i = 0; i < numTargets(); i++) {
            targetPositions[i]->write(
                file, va("target_%s", targetPositions[i]->typeStr()));
        }
...
void idCameraDef::addTarget(const char* name,
    ...
    // TTimo: unused
    // const char *text = (name == NULL) ? va("target0%d", numTargets()+1) :
    // name;
