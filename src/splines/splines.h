...
public:
    ...
    // this can be done with RTTI syntax but i like the derived classes setting
    // a type makes serialization a bit easier to see
    //
...
public:
    ...
    virtual void draw(bool editMode) {
        glLabeledPoint(blue, startPos, (editMode) ? 5 : 3,
                       "Start interpolated");
...
public:
    ...
    void draw(bool editMode) {
        ...
        if (cameraPosition) {
            cameraPosition->draw(
                (bool)((editMode || cameraRunning) && cameraEdit));
